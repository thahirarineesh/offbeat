...
def add_to_cart_from_wishlist(request, wishlist_id):
    ...
    existing_cart_item = Cart.objects.filter(product=product, user=request.user).first()

    print("Existing cart item:", existing_cart_item)  # Debug message

    if existing_cart_item:
        existing_cart_item.quantity += 1  # Increase quantity if already in cart
        existing_cart_item.save()
    else:
        # Create a new cart item if not in cart
        Cart.objects.create(
            product=product,
            user=request.user,
            quantity=1
